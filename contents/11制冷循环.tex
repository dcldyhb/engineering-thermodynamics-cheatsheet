...
\pt{制冷系数}在工程上也称制冷装置的工作性能系数，即 $\mathrm{COP}$。$\varepsilon=\frac{q_\mathrm{c}}{q_0-q_\mathrm{c}}=\frac{1}{\pi^\frac{\kappa-1}{\kappa}-1}=\frac{T_1}{T_2-T_1}$ ，若为卡诺循环 $\varepsilon_\mathrm{c}=\frac{T_\mathrm{c}}{T_0-T_\mathrm{c}}$，有 $\varepsilon_\mathrm{c}>\varepsilon$

深冷装置通常 $\varepsilon<1$，普通制冷装置通常 $\varepsilon>1$。

$\pi\downarrow$，$\varepsilon\uparrow$，但制冷量 $q_\mathrm{c}$ 下降。
